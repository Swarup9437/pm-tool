...
router.use(
  ['/employees/new','/employees', '/employees/:id/edit', '/employees/:id/delete'],
  requireAuth
);
...
router.get('/employees/new', (req, res) => {
  res.send(`
    ...
    <form method="POST" action="/employees">
      ...
      <div><label>Temp Password: <input name="password" type="password" placeholder="optional (min 6 chars)"></label></div>
...
router.post(
  ...
  [
    ...
    body('role').isIn(['admin','pm','engineer','viewer']).withMessage('Invalid role'),
...
// Save edits (validated)
router.post(
  '/employees/:id/edit',
  [
    body('name').trim().notEmpty(),
    body('email').isEmail(),
    body('role').isIn(['admin','pm','engineer','viewer']),
    body('phone').optional().isString().trim(),
